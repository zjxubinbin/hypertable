    ...
    m_error(Error::OK), m_cancelled(false), m_use_index(false)
{
  ...
  if (!(flags&Table::SCANNER_FLAG_IGNORE_INDEX)
    ...
    cb = new IndexResultCallback(table, scan_spec, cb, timeout_ms, 
                use_qualifier);

    // get the index table
    if (use_qualifier)
      table = &(*table->get_qualifier_index_table());
    else
      table = &(*table->get_index_table());

    // fall through and retrieve values from the primary table. the 
    // IndexResultCallback object will then transform the results and forward 
                ...
                ScanSpecBuilder &index_spec, bool *use_qualifier)
{
  ...
  if (primary_spec.columns.size() && !(primary_spec.columns.size() & 1)) {
    // hack alert: this is a workaround for issue #738
    for (size_t i = 0; i < primary_spec.columns.size(); i += 2) {
      Schema::ColumnFamily *cf = 0;
      const char *qualifier = strchr(primary_spec.columns[i + 1], ':');
      if (!qualifier)
        goto skip_qualifier;

      String column(primary_spec.columns[i + 1], qualifier);
...
skip_qualifier:
  ...
  *use_qualifier = false;
  ...
  if (primary_spec.column_predicates.size() && primary_spec.columns.size()) {
    ...
    HT_ASSERT(!strcmp(primary_spec.column_predicates[0].column_family,
                      primary_spec.columns[0]));
