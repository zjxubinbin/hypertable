...
void create_index_in_hyperspace(ContextPtr &context, const String &name,
                                const String &schema_str, bool qualifier,
                                TableIdentifierManaged *index_table)
{
  ...
  // index with ^^
  String index_name = Filesystem::dirname(name);
  if (qualifier) {
    if (index_name == "/")
  ...
  String s;
  index_schema->render(s);
  delete index_schema;

  // now create the table in hyperspace
  create_table_in_hyperspace(context, index_name, s, index_table);
}
