...
### Secondary Indices
<p>
Tables can also consist of one or more indices, each indexing a single 
column family.  Two types of indices exist: a cell value index, which 
optimizes scans by cell value, and a qualifier index, which 
optimizes scans for cells with a column qualifier.  The use of 
indices is optional.
<p>
...
<p>
Indices speed up read access when querying data from a column family.
